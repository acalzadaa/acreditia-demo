<script lang="ts">
	...
	interface Props {
		selectedItem: EtapaAutorizacionPlanMejoraItem;
		open: boolean;
		onClose: () => void;
	}
	...
	let formData = $derived({
		authorized: selectedItem.authorized
	});
...
<Modal bind:open onClickClose={handleClose} closeOnEscape closeOnBackdropClick>
	<div class="modal">
		<header class="modal-header">
			<h2 class="text-h4">Autorizar plan de mejora</h2>
		</header>
		...
		>
				...
				<div class="modal-body">
					<div class="confirm-content">
						<p class="confirm-message text-body-large">
							¿Esta seguro que desea autorizar el plan de mejora ?
						</p>
					</div>
				</div>
			</div>
			...
			<footer class="modal-footer text-body">
				<Button type="submit" variant="primary" isDisabled={isSubmitting}>
					{isSubmitting ? 'Guardando...' : 'Autoriza plan'}
				</Button>
			</footer>
		</form>
	</div>
