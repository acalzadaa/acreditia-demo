<script lang="ts">
	...
	interface Props {
		open: boolean;
		indicadorEstrategicoRef: IndicadorEstrategicoRef[];
		onClose: () => void;
	}

	let { open = $bindable(false), onClose, indicadorEstrategicoRef = [] }: Props = $props();
...
<Modal bind:open onClickClose={handleClose} closeOnEscape closeOnBackdropClick>
	<div class="modal">
		<header class="modal-header">
			<h2 class="modal-title text-h4">Agregar indicador estrategico</h2>
			<IconButton
				name="close"
				variant="ghost"
				size="lg"
		...
		>
			...
			<footer class="modal-footer text-body">
				...
				<Button type="submit" variant="primary">Agregar indicador</Button>
			</footer>
		</form>
	</div>
</Modal>
