<script lang="ts">
	...
	import { areaFuncionalItemSchema } from '$lib/schemas/areaFuncional.schema';
	import type { AreaFuncionalRef } from '$lib/schemas/shared.schema';

	interface Props {
		open: boolean;
		selectedItem: AreaFuncionalRef;
		onClose: () => void;
	}
	...
	const { form, enhance } = superForm(selectedItem, {
		dataType: 'json',
		validators: zod4(areaFuncionalItemSchema),
...
<Modal bind:open onClickClose={handleClose} closeOnEscape closeOnBackdropClick>
	<div class="modal">
		<header class="modal-header">
			<h2 class="modal-title text-h4">Remover subarea</h2>
			<IconButton
				name="close"
				variant="ghost"
				size="lg"
