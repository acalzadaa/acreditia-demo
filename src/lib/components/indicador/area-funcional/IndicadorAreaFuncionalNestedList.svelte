<script lang="ts">
	...
	interface Props {
		...
		onClickRemover: (item: IndicadorAreaFuncionalItem) => void;
					...
					>
						{#snippet title()}
							<div style="display: flex; flex-direction: row; align-items: center;">
								<p style="text-transform: capitalize;">{item.areaFuncional.name}</p>
								<Tag variant="info">{item.areaFuncional.code}</Tag>
								<Actions
									{item}
						...
						<Button
							...
							size="sm"
							name="add"
							onClick={() => onClickAddChild({ parentId: item.areaFuncional.id })}
						>
							Agregar subarea funcional
						</Button>
					</AccordionHeaderButton>

					<AccordionContent isCollapsible={true} isVisible={accordions.isOpen(item.id)}>
						{#each item['subareasFuncionales'] as subareasFuncional (subareasFuncional.id)}
							<AccordionContentItem
								label={subareasFuncional.code}
								value={subareasFuncional.name}
								onRemove={() =>
									onClickRemoverChild({ parentId: item.id, childId: subareasFuncional.id })}
								removeIcon="remove"
