...
class AuthStore {
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      this.unsubscribe = netlifyIdentity.onAuthChange(
        (event: AuthEvent, user: User | null) => {
  ...
  async signUp(email: string, password: string, metadata?: Record<string, unknown>): Promise<AuthOperationResult> {
    try {
      this.state.loading = true;
